...
6. Optionally, for **Secret** enter a signing secret.  If you don't provide one, Unstructured generates a random secret for you.
...
- Create and rotate the secret for a webhook. You can optionally provide a secret when you create a webhook. If you do not provide one, Unstructured generates a random secret for you. You can also change the secret by using the API. Use the secret to verify that a webhook originated from Unstructured. If you suspect that the secret has been compromised, rotate it to a new value by using the API.
