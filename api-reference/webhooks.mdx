...
- Optionally, replace `<secret>` with the signing secret in plain text for the notification channel. If you do not provide a secret, Unstructured generates a random secret for you. The secret must be between 24 and 75 bytes. If you use only ASCII characters, this is equivalent to 24 to 75 characters, because each ASCII character is one byte.
...
call the `POST /notifications/channels/<channel-id>/secret/rotate` operation.
...
<AccordionGroup>
    <Accordion title="curl">
        ```bash
        curl --request 'POST' --location \
        "$UNSTRUCTURED_API_URL/notifications/channels/<channel-id>/secret/rotate" \
        --header "unstructured-api-key: $UNSTRUCTURED_API_KEY"
        ```
    </Accordion>
    <Accordion title="Postman">
...
- Replace any or all of the following settings with the new settings you want to use for the notification channel:
  ...
  - `<secret>`: A new signing secret in plain text for the notification channel. Omit `secret` to keep the current value. The secret must be between 24 and 75 bytes. If you use only ASCII characters, this is equivalent to 24 to 75 characters, because each ASCII character is one byte.  
...
call the `POST /workflows/<workflow-id>/notifications/channels/<channel-id>/secret/rotate` operation.
...
<AccordionGroup>
    <Accordion title="curl">
        ```bash
        curl --request 'POST' --location \
        "$UNSTRUCTURED_API_URL/workflows/<workflow-id>/notifications/channels/<channel-id>/secret/rotate" \
        --header "unstructured-api-key: $UNSTRUCTURED_API_KEY"
        ```
    </Accordion>
    <Accordion title="Postman">
