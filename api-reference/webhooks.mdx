...
completed means that the job has initialized and finished it's work but it does not mean all files were successfully processed. User would need to fetch job details after job completes, and look at processed documents count to have the whole context of the job. (I think we should add this information somewhere in the docs)
...
- Optionally, replace `<secret>` with the signing secret in plain text for the notification channel. If you do not provide a secret, Unstructured generates a random secret for you. The secret must be between 24 and 75 bytes (24 to 75 ASCII characters).
